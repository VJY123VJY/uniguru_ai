...
class SovereignEnforcement:
    ...
    def process_and_seal(self, decision_schema: Dict[str, Any], request_id: str) -> Dict[str, Any]:
        ...
        if not v_status:
            truth_decl = str(verification_meta.get("truth_declaration", "")).upper()
            if truth_decl == "VERIFIED":
                v_status = "VERIFIED"
            elif truth_decl in ["VERIFIED_PARTIAL", "PARTIAL"]:
                v_status = "PARTIAL"
            elif decision_schema.get("decision") in ["forward", "fallback"]:
                v_status = "PARTIAL"
            else:
                v_status = "UNVERIFIED"
        ...
        if v_status == "VERIFIED":
            ...
            prefix = self._resolve_declaration(
                verification_meta,
                default_source="UniGuru Reasoning",
                partial=True,
            )
            decision_schema["verification_prefix"] = prefix
            ...
            # IMPORTANT: Never block a 'fallback' or 'forward' response
            if decision_schema.get("decision") in ["fallback", "forward"]:
                decision_schema["status_action"] = "ALLOW_WITH_CAUTION"
                decision_schema["verification_status"] = "PARTIAL"
                prefix = "Verification status: UNVERIFIED"
                decision_schema["verification_prefix"] = prefix
                data["response_content"] = self._prefix_if_missing(content, prefix)
            else:
