...
def _fallback_knowledge_answer(query: str) -> Optional[Dict[str, Any]]:
    """Secondary retrieval from keyword index and markdown knowledge when Kosha signals fail."""
    if SignalValidator.is_off_topic_query(query):
        return None

    from retrieval.retriever import _try_markdown_search, _try_keyword_kb

    for retrieve_fn in (_try_keyword_kb, _try_markdown_search):
        ...
        if content and trace.get("match_found"):
            ...
            return {
                ...
                "reasoning": f"Accepted via {trace.get('method')} after Kosha signal validation found no match.",
            }

    kb = kb_retrieve(query)
    answer = str(kb.get("answer") or "").strip()
    if kb.get("verified") and answer and answer != NO_KNOWLEDGE_RESPONSE:
        return {
            "answer": answer,
            "verification_status": "VERIFIED",
            "confidence": float(kb.get("confidence_level") or 0.0),
            "source": kb.get("source_file"),
            "method": "keyword_kb",
            "reasoning": "Accepted via keyword knowledge index after Kosha signal validation found no match.",
        }
    return None
...
def run_deterministic_pipeline(
    ...
    if synthesis["verification_status"] == "NO_VERIFIED_KNOWLEDGE":
        ...
        fallback = _fallback_knowledge_answer(query)
        if fallback:
            payload = {
                "trace_id": trace_id,
                "query": query,
                ...
                "reasoning_path": reasoning_path[:-1] + ["knowledge_index_fallback", "deterministic_synthesis"],
                "confidence_breakdown": {
                    "overall": round(float(fallback["confidence"]), 4),
                    "accepted_count": 1,
                ...
                "synthesis_mode": "FALLBACK_KNOWLEDGE_INDEX",
                "confidence": round(float(fallback["confidence"]), 4),
                "signals_found": 0,
