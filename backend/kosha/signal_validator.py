...
OFF_TOPIC_PATTERNS = (
    ...
    r"\bleaking faucet\b",
    r"\bmovie to watch\b",
    r"\btime travel\b.*\bpossible\b",
    r"\bquantum entanglement\b.*\bbhagavad gita\b",
)
...
MIN_SIGNAL_CONFIDENCE = 0.12
MIN_TAG_OVERLAP = 1  # At least 1 tag must match query
MIN_SEMANTIC_SCORE = 0.22
MIN_CONCEPT_OVERLAP = 0.10
MIN_ENTITY_OVERLAP_WHEN_ENTITY_QUERY = 0.35
...
class SignalValidator:
    ...
    def validate_signal(
        ...
        # Accept if tag match, content overlap, OR ontology-aware semantic alignment
        has_tag_match = len(matched_tags) >= MIN_TAG_OVERLAP
        has_content_match = content_overlap > 0.08
        has_semantic_match = (
            float(details["semantic_score"]) >= MIN_SEMANTIC_SCORE
            and float(details.get("concept_overlap") or 0.0) >= MIN_CONCEPT_OVERLAP
        )

        if not has_tag_match and not has_content_match and not has_semantic_match:
            return False, "no_query_relevance:tags_content_semantic_miss", details

        # Semantic-only acceptance must tie to entities or strong concept overlap
        if not has_tag_match and not has_content_match and has_semantic_match:
            entity_overlap = float(details.get("entity_overlap") or 0.0)
            concept_overlap = float(details.get("concept_overlap") or 0.0)
            if entity_overlap <= 0.0 and concept_overlap < 0.18:
                return False, "semantic_only_without_entity_or_concept", details
        ...
        expected_domain = (details.get("domain_resolution") or {}).get("domain", "general")
        if expected_domain != "general" and float(details["domain_consistency"]) <= 0.0:
        ...
        if (
            ...
            and content_overlap <= 0.08
            and float(details["contextual_proximity"]) < 0.35
        ):
            return False, "low_contextual_overlap", details
    ...
    @classmethod
    def is_off_topic_query(cls, query: str) -> bool:
        text = query.lower()
    ...
    def validate_all(
        ...
        """
        if cls.is_off_topic_query(query):
            return {
                "query": query,
                "query_tokens": list(cls.tokenize(query)),
                "signals_found": 0,
                ...
                "rejected_signals": [
                    {
                        ...
                        "rejection_reason": "off_topic_query",
                        "details": {},
                    }
                    for signal in signals
                ],
                "has_valid_knowledge": False,
            }
